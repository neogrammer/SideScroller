...
std::unique_ptr<sf::Sprite> phys::spr(rec& r)
{
	...
	if (r.texType != Cfg::Textures::Count)
	{
		...
		sprite->setTextureRect(r.getTexRect());
	}
...
bool phys::PointVsRect(const sf::Vector2f& p, const rec& r)
{
...
bool phys::RayVsRect(const sf::Vector2f& ray_origin, const sf::Vector2f& ray_dir, const rec& target, sf::Vector2f& contact_point, sf::Vector2f& contact_normal, float& t_hit_near)
{
...
bool phys::DynamicRectVsRect(const rec& in, const rec& target, sf::Vector2f& contact_point, sf::Vector2f& contact_normal, float& contact_time, float fElapsedTime)
{
